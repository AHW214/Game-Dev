...
namespace PlatformerFSM
{
    public class CameraFollow : MonoBehaviour
    {
        ...
        private void Start()
        {
            focusArea = new FocusArea(player.controller.collider.bounds, focusAreaDimensions);
        }

        private void LateUpdate()
        {
            focusArea.Update(player.controller.collider.bounds);
            ...
            if (focusArea.displacement.x != 0)
            {
                ...
                if (Math.Sign(player.input.x) == Math.Sign(focusArea.displacement.x))
                {
